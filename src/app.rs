...
impl EditingSession {
    ...
    fn handle_editing_command(&mut self, command: EditorCommand) -> Result<(), DocumentError> {
        match command {
            ...
            // `ShowHelp` is unreachable here: `handle_command` opens the help dialog
            // as a global overlay before dispatching to mode-specific handlers.
            ...
            PromptState::SaveConflict {
                ...
                EditorCommand::Enter => match focus {
                    ConflictChoice::Reload => {
                        ...
                        // The rope was replaced wholesale, so any recorded EditSteps
                        // reference char indices and text of the *old* document.
                        // Applying them would corrupt the new rope — discard them.
                        self.history.clear();
    ...
    fn delete_or_backspace(&mut self, forward: bool) {
        if let Some(selection) = self.selection
            && !selection.is_empty()
        {
