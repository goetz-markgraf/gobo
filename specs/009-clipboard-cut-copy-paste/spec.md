...
**Why this priority**: Copy ist die Grundlage für das Wiederverwenden von Text ohne Datenverlust. Ohne diese Funktion ist manuelles Kopieren (Markieren, Ausschneiden, manuelches Einfügen, Rückgängigmachen) der einzige Weg — ineffizient und fehleranfällig.
...
1. **Given** der Text "Hallo Welt" mit selektiertem Teil "Welt", **When** der Nutzer Ctrl-C drückt, **Then** steht im System-Clipboar der Text "Welt", die Selektion bleibt bestehen, und der Editor-Inhalt verändert sich nicht.
...
**Independent Test**: Cursor zwischen zwei Zeichen positionieren (oder am Dokumentanfang/-ende), Ctrl-C drücken, Editor-Inhalt prüfen und Undo auslösen. Erwartete Wirkung: Zeichen verschwindet mit einem Undo vollständig in der ursprünglichen Position.

**Acceptance Scenarios**:

1. **Given** der Text "HalloWelt" mit Cursor zwischen "o" und "W", **When** der Nutzer Ctrl-X drückt, **Then** steht im System-Clipboar "W" und im Editor nur noch "Halloelt".
...
**Why this priority**: Das Verschieben von Textblöcken ist eine der häufigsten编辑-Aktionen; ohne Cut müsste der Nutzer manuell löschen, zur Zielposition springen und einfügen — fehleranfällig bei langen Dokumenten.
...
3. **Given** eine Selektion über mehrere Zeichen einer einzelnen Zeile ("Hallo Welt", selektiert "lo W"), **When** der Nutzer Ctrl-X drückt, **Then** steht im System-Clipboar "lo W" und im Editor nur noch "Halt Welt".
...
Ein Nutzer hat einen Textbereich markiert und möchte ihn durch den Clipboard-Inhalt ersetzen (überschreiben). Durch Drücken von Ctrl-V wird der gesamte selektierte Bereich im Editor gelöscht und der Clipboard-Inhalt an seiner Stelle eingefügt. Dieser Ersatz Vorgang ist atomar: ein einzelnes Undo stellt sowohl den vorherigen selektierten Text als auch die umgebenden Textteile wieder her.
...
**Independent Test**: Einen Bereich markieren, Clipboard beinhalten "Korrektur", Ctrl-V auslösen, prüfen dass Markierung durch Clipboard-Inhalt ersetzt wurde. Undo wiederherstellen. Erwartete Wirkung: selektierter Bereich wird vollständig durch Clipboard-Inhalt ersetzt; Undo stellt Originaltext her.
...
- **FR-005**: Alle Cut- und Paste-Vorgänge (Ctrl-X, Ctrl-C, Ctrl-V) MUSS als einzelne atomare Einheit im Undo-Verlauf aufgezeichnet werden; ein einzelnes Ctrl-Z stellt den Zustand vor der Aktion vollständig wieder her.
...
- **FR-007**: Ein Paste-Vorgang MUSS die Selektion (falls vorhanden) nach dem Einfegen aufheben und den Cursor hinter dem eingefügten Text positionieren.
...
- **FR-013**: Alle Copy/Cut/Paste-Vorgänge MÜSSEN den Clipboard-Inhalt auf maximal 1 MB beschränken. Größere Inhalte werden abgelehnt und der Nutzer mit einem nicht-blockierenden Hinweis informiert.
...
- **SC-003**: Ein einziger Paste-Vorganf mit Ctrl-V setzt den System-Clipboard-Inhalt an genau der Cursorposition ein (oder ersetzt die Selektion); Undo entfernt alles in einem Schritt, ohne Resteffekte.
- **SC-004**: Das System-Clipboad wird nach jedem Cut oder Copy aktualisiert; nachfolgende Einfügeoperationen verwenden den aktuellsten Inhalt; frühere Clipboard-Einträge bleiben bestehen solange sie nicht überschrieben werden.
- **SC-05**: Der Editor MUSS keine Abstürze, Hänger oder Datenverluste verursachen, wenn das System-Clipboard leeren, binären oder sehr großen Inhalt enthält (robuste Handhabung von Edge Cases).
