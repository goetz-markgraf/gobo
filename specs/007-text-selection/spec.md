...
- **FR-002**: Der Editor MUST die Selektionsrichtung korrekt abbilden und beibehalten. Selektionsrichtung ist hierbei als Index-Relation definiert (vorwärts, wenn `Head >= Anchor`, sonst rückwärts) und nicht als Ausrichtung am Textfluss oder Dokumentende. Aufeinanderfolgende Shift-Bewegungen MUST die Selektion konsistent wachsen oder schrumpfen lassen, auch über den Anker hinaus, wobei die Index-Relation sich natürlich umkehrt, ohne dass spezialbehandelter Code erforderlich ist.
...
- **FR-011**: Nicht editierende Befehle (z. B. Suche, Speichern, Quit-Prompt) MAY die Selektion nicht unbeabsichtigt verändern; Modswechsel oder Modus-/Kontextwechsel dürfen die Selektion nur dann verwerfen, wenn es der Wechsel ausdrücklich erfordert. Explizit erlaubte Selektion-verwerfende Trigger sind: plain `Move*` ohne Shift (FR-004), editierende Aktionen nach Verbrauch der Selektion (FR-005/FR-006), `Undo`/`Redo` (FR-015) sowie ein `Reload` des Dokumentinhalts (z. B. `SaveConflictPrompt → Reload`), bei dem der Cursor als Teil des Resets an den Dokumentanfang zurückgesetzt wird.
...
- **FR-015**: `Undo` (Ctrl-Z) und `Redo` (Ctrl-Y) MUST eine aktive Selektion aufheben (`selection = None`), da der durch Undo/Redo wiederhergestellte Textinhalt keine sinnvolle Selektion besitzt. Separat davon bleibt die atomare Einzelschritt-Garantie aus FR-007 für die zuvor aufgezeichnete Aktion erhalten.
...
- Befehle, die nicht in den Editiermodus fallen (Suche, Speichern, Quit-Prompt), werden nach heutigem Verhalten priorisiert; sie lassen die Selektion unangetastet, es sei denn, der Moduswechsel erfordert dies ausdrücklich. `Undo`/`Redo` heben die Selektion auf (FR-015); ein `Reload` des Dokumentinhalts (z. B. `SaveConflictPrompt → Reload`) verwirft die Selektion als Teil des Cursor-Resets, `Overwrite`/`Cancel` belassen die Selektion unverändert.
